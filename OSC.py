    ...
    def dispatchMessage(self, pattern, tags, data, client_address):
        ...
        if len(tags) != len(data):
            raise OSCServerError(
                "Malformed OSC-message; got %d typetags [%s] vs. %d values" % (len(tags), tags, len(data)))
        ...
        for addr in self.callbacks.keys():
            ...
            if match and (match.end() == len(addr)):
                ...
                if isinstance(reply, OSCMessage):
                    ...
                    raise TypeError("Message-callback %s did not return OSCMessage or None: %s" % (
...
class OSCServer(UDPServer, OSCAddressSpace):
    ...
    def serverInfo_handler(self, addr, tags, data, client_address):
        ...
        if cmd in ('help', 'info'):
            ...
            self.reportErr("unrecognized command '%s' in /info request from osc://%s. Try 'help'" % (
            cmd, getUrlStr(client_address)), client_address)
    ...
    def subscription_handler(self, addr, tags, data, client_address):
        ...
        if not isinstance(self.client, OSCMultiClient):
            raise OSCServerError(
                "Local %s does not support subsctiptions or message-filtering" % self.client.__class__.__name__)
        ...
        if len(data):
            if data[0] in ('help', 'info'):
                ...
                reply.append(('subscribe_command',
